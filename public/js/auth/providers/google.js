/**
 * =================================================================
 * GOOGLE AUTH PROVIDER DEFINITION
 * =================================================================
 *
 * Self-contained provider config for Google Sign-In.
 * Dynamically imports the Google auth module at registration time.
 *
 * To register: import and call registerGoogleProvider()
...
/**
 * Register the Google provider with the auth registry.
 * Dynamically loads the auth module to avoid eager imports.
 */
export async function registerGoogleProvider() {
    try {
        const module = await import('../google-auth.js');
        registerAuthProvider({
            ...googleProvider,
            login: module.loginWithGoogle,
            logout: async () => true // Google logout handled by backend signOut
        });
    } catch (err) {
